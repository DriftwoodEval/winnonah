...
echo "Starting cron schedule using: $CRON_SCHEDULE, $NOTIFY_CRON_SCHEDULE, $CATEGORIZATION_CRON_SCHEDULE, $BABYNET_REPORT_CRON_SCHEDULE, $OFFICE_DRIVE_TIMES_CRON_SCHEDULE, and $REFERRAL_STATUS_FAX_CRON_SCHEDULE"
...
echo "$REFERRAL_STATUS_FAX_CRON_SCHEDULE cd /app && uv run referral_status_faxes.py" >> /tmp/crontab
