...
echo "Starting cron schedule using: $CRON_SCHEDULE, $NOTIFY_CRON_SCHEDULE, $INFO_REQUEST_CRON_SCHEDULE, and $CATEGORIZATION_CRON_SCHEDULE"
...
echo "$INFO_REQUEST_CRON_SCHEDULE cd /app && uv run info_request_intake.py" >> /tmp/crontab
