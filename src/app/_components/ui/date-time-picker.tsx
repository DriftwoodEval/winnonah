...
// as an "outside" interaction, closing the whole popover before a selection
// can be made. Ignore those so only real outside clicks dismiss it.
        ...
        <InputGroupAddon align="inline-end">
          <Popover
            onOpenChange={(nextOpen, eventDetails) => {
              if (!nextOpen && isNativeSelectInteraction(eventDetails)) {
                eventDetails.cancel();
                return;
              }
              setOpen(nextOpen);
            ...
            open={open}
          >
            <PopoverTrigger render={<InputGroupButton variant="ghost"
            size="icon-xs"
            disabled={disabled}
            aria-label="Open calendar"><CalendarIcon className="h-4 w-4" /></InputGroupButton>} />
            <PopoverContent
              align="end"
              side="bottom"
              sideOffset={4}
