...
export function QSuiteTab() {
	...
	const defaultTab = canEditGeneral
		? "general"
		: canEditServices
			? "services"
			: canEditRecords
				? "records"
				: "piecework";
	const activeTab = searchParams.get("qsuiteTab") ?? defaultTab;

	const handleTabChange = (value: string) => {
		const params = new URLSearchParams(searchParams.toString());
		params.set("qsuiteTab", value);
				...
				<Tabs onValueChange={handleTabChange} value={activeTab}>
					<div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
						<TabsList className="!h-auto flex-wrap justify-start gap-1">
							...
							{canEditPiecework && (
								<TabsTrigger value="piecework">Piecework</TabsTrigger>
							)}
						</TabsList>
						<Button
							disabled={
					...
					{canEditRecords && (
						<TabsContent className="mt-6" value="records">
							<RecordsTab disabled={!canEditRecords} form={form} />
						</TabsContent>
					)}
					{canEditPiecework && (
						<TabsContent className="mt-6" value="piecework">
...
type DistrictOption = { id: number; name: string; isCharter: boolean };
...
function RecordsTab({
	...
	const districtOptions = useMemo<DistrictOption[]>(() => {
		...
		for (const dist of allSchoolDistricts ?? []) {
			...
			seen.add(name);
			opts.push({ id: dist.id, name, isCharter: dist.isCharter });
		}
		...
		<div className="grid gap-6">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Map of school districts and charter schools to emails to send
						records requests to.
					</CardDescription>
				</CardHeader>
				<CardContent>
