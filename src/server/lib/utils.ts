...
export const sortClients = (clients: SortedClient[]) => {
  ...
  const clientsWithBabyNetAboveAge =
    getBabyNetClientsAboveAge(clientsWithBabyNet);

  const remainingClientsWithBabyNet = clientsWithBabyNet.filter(
    (client) =>
      !clientsWithBabyNetAboveAge.some(
        (clientAboveAge) => clientAboveAge.id === client.id
      )
  );

  const remainingClients = clients.filter(
    (client) =>
      !clientsWithBabyNet.some(
        (clientWithBabyNet) => clientWithBabyNet.id === client.id
      )
  );
