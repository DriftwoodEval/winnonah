...
export const schoolDistricts = createTable("school_district", (d) => ({
	...
	// Charter schools are stored in the same table so they appear in the
	// QSuite records-contact picker and resolve through the same
	// config.records_emails map. records-request.py uses this flag to tell a
	// charter-school consent form from a public-district one.
	isCharter: d.boolean().notNull().default(false),
}));
...
export const clientInsurancePolicies = createTable(
	...
	(d) => ({
		policyId: d.varchar({ length: 36 }).notNull().primaryKey(),
		clientId: d
			.int()
			.notNull()
			.references(() => clients.id, { onDelete: "cascade" }),
		...
		insuranceNumber: d.varchar({ length: 255 }),
		groupNumber: d.varchar({ length: 255 }),
		employer: d.varchar({ length: 255 }),
		policyCompanyName: d.varchar({ length: 255 }),
		policyCompanyPhone: d.varchar({ length: 255 }),
		memo: d.varchar({ length: 1000 }),
		clientMemo: d.varchar({ length: 1000 }),
		insuranceType: d.varchar({ length: 255 }),
		insuranceCompanyName: d.varchar({ length: 255 }),
		insurancePayerId: d.varchar({ length: 255 }),
		insurancePhone: d.varchar({ length: 255 }),
		insurancePrecertPhone: d.varchar({ length: 255 }),
		insuranceWebsite: d.varchar({ length: 255 }),
		insuranceAddress1: d.varchar({ length: 255 }),
		insuranceAddress2: d.varchar({ length: 255 }),
		insuranceAddress3: d.varchar({ length: 255 }),
		insuranceCity: d.varchar({ length: 255 }),
		insuranceState: d.varchar({ length: 50 }),
		insuranceZip: d.varchar({ length: 20 }),
		insuranceCountry: d.varchar({ length: 100 }),
		insuredFirstName: d.varchar({ length: 255 }),
		...
		insuredAddress3: d.varchar({ length: 255 }),
		insuredCity: d.varchar({ length: 255 }),
		insuredState: d.varchar({ length: 50 }),
		insuredZip: d.varchar({ length: 20 }),
		insuredCountry: d.varchar({ length: 100 }),
		insuredRelation: d.varchar({ length: 100 }),
		insuredRelationOther: d.varchar({ length: 255 }),
		referRole: d.varchar({ length: 255 }),
		referName: d.varchar({ length: 255 }),
		referNpi: d.varchar({ length: 50 }),
		benefitsCpt: d.varchar({ length: 255 }),
		deductible: d.varchar({ length: 100 }),
		deductibleMet: d.varchar({ length: 100 }),
		paysAt: d.varchar({ length: 100 }),
		isCopay: d.boolean(),
		precertRequired: d.boolean(),
	...
	(t) => [index("cip_client_idx").on(t.clientId)],
);

export const clientInsurancePoliciesRelations = relations(
	clientInsurancePolicies,
	({ one }) => ({
		client: one(clients, {
			fields: [clientInsurancePolicies.clientId],
			references: [clients.id],
		}),
	}),
);
