...
export interface ClientInfoData {
	...
	mostRecentAppointment: string | undefined;
	mostRecentAppointmentProvider: string | null | undefined;
	questionnaires: {
		type: string | null;
		status: string | null;
		sent: string | null;
...
/**
 * Assembles the same client-summary fields the weekly appointment-agenda
 * email and the internal client-info API both need: client details, records
 * status, BabyNet status, questionnaire list, and the client note. Shared so
 * neither caller drifts from the other.
 */
export async function getClientInfoData(
	clientId: number,
): Promise<ClientInfoData | null> {
	...
	const matchedTemplate = NOTE_TEMPLATES.find((t) => fullNote.includes(t.text));
	...
	let recordsStatus: string | false = false;
	...
	let babyNetERStatus: string | false = false;
	if (client.babyNetERNeeded) {
		babyNetERStatus = client.babyNetERDownloaded
			? "Downloaded"
